...
export const sendDailyNewsEmail = async ({email, newsContent}: {email: string, newsContent: string}) => {
    const htmlTemplate = NEWS_SUMMARY_EMAIL_TEMPLATE.replace('{{newsContent}}', newsContent);

    const mailOptions = {
        from: `"NextTrade <News@NextTrade.com>"`,
        to: email,
        subject: `Daily Market Insights: Stay Ahead of the Curve`,
